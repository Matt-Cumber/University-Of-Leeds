...
void addChildIfNotIndicator(Node *node, double tolerance, int choice,int *countFalse) {
  ...
  if(node->child[0]==NULL){
    ...
    for(i=0;i<4;++i)
      addChildIfNotIndicator(node->child[i], tolerance, choice, countFalse);
  }
}

void taskFourAlgorithm(Node *node, double tolerance, int choice) {
  int countFalse=0;
  addChildIfNotIndicator( node,tolerance,choice,&countFalse);
  if(countFalse==0) return;
  else{
    taskFourAlgorithm(node,tolerance,choice);
  }
}
